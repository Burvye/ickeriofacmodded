    ...
    public interface Suggests {
        ...
        static SuggestionProvider<ServerCommandSource> allPlayers() {
            return (context, builder) -> {
                ...
                for (User user : User.all()) {
                    // Prefer online player name
                    ServerPlayerEntity online = server.getPlayerManager().getPlayer(user.getID());
                    if (online != null) {
                        builder.suggest(online.getGameProfile().name()); // 1.21.9 uses name()
                    } else {
                        // Fallback to UUID if offline (no cache needed)
        ...
        static SuggestionProvider<ServerCommandSource> allPlayersInYourFactionButYou() {
            return (context, builder) -> {
                ...
                User currentUser = User.get(self.getUuid());
                ...
                for (User user : User.all()) {
                    ...
                    ServerPlayerEntity online = server.getPlayerManager().getPlayer(user.getID());
                    if (online != null) {
                        builder.suggest(online.getGameProfile().name()); // 1.21.9
                    } else {
                        builder.suggest(user.getID().toString());
                    }
                }
        ...
        public static <T extends Enum<T>> SuggestionProvider<ServerCommandSource> enumSuggestion(
                Class<T> clazz) {
            return suggest(
                    user ->
