    ...
    public InfoGui(ServerPlayerEntity player, Faction faction, @Nullable Runnable closeCallback) {
        ...
        String owner =
                members.stream()
                        ...
                        .map(u -> {
                            ServerPlayerEntity online = server.getPlayerManager().getPlayer(u.getID());
                            return online != null
...
// Power info — power needed to KEEP current claims (no +1)
        final int claimWeight   = FactionsMod.CONFIG.POWER.CLAIM_WEIGHT;
        final int requiredPower = FactionsSafe.claimCount(faction) * claimWeight;
        final int maxPower      = faction.calculateMaxPower();

// Optional: color it red if under the requirement
        final boolean ok   = faction.getPower() >= requiredPower;
        final Style reqSty = Style.EMPTY.withItalic(false).withColor(ok ? Formatting.GREEN : Formatting.RED);
        this.setSlot(
                2,
                new GuiElementBuilder(Items.PLAYER_HEAD)
                        ...
                        .setLore(
                                List.of(
                                        ...
                                        Text.translatable(
                                                        ...
                                                        Text.literal(String.valueOf(requiredPower)).setStyle(reqSty)
                                                                .setStyle(
                                                                        Style.EMPTY
                                                                                .withItalic(false)
                                                                                .withColor(
