...
public class AdminGui extends SimpleGui {
    ...
    // …add this helper method inside AdminGui …
    private GuiElementBuilder bypassButton(ServerPlayerEntity player, User user) {
        return new GuiElementBuilder(Items.PLAYER_HEAD)
                ...
                .setCallback((slotIndex, clickType, actionType) -> {
                    GuiInteract.playClickSound(player);
                    user.bypass = !user.bypass;

                    // re-render the same slot with updated state
                    this.setSlot(slotIndex, bypassButton(player, user));

                    new Message(Text.translatable("factions.gui.admin.options.bypass.success"))
                            .filler("·")
                            .add(new Message(
                                    user.bypass ? Text.translatable("options.on")
    ...
    public AdminGui(ServerPlayerEntity player) {
        ...
        // Bypass icon
        this.setSlot(indexes.get(0), bypassButton(player, user));
    ...
    private void execSpoof() {
        ...
        inputGui.confirmBtn.setCallback(
                (index, clickType, actionType) -> {
                    ...
                    ServerPlayerEntity online = player.getEntityWorld().getServer().getPlayerManager().getPlayer(input);
                    UUID id = null;
                    if (online != null) id = online.getUuid();
                    else {
                        try { id = UUID.fromString(input); } catch (IllegalArgumentException ignored) {}
                    }
                    if (id == null) {
                        inputGui.showErrorMessage(
                                Text.translatable("factions.gui.spoof.fail.no_player", input).formatted(Formatting.RED),
                                index);
                        return;
                    }
                    assert online != null;
                    User target = User.get(online.getUuid());
