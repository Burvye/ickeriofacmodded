...
public class InfoCommand implements Command {
    ...
    public static int info(ServerPlayerEntity player, Faction faction) {
        ...
        List<User> users = faction.getUsers();
            // how much power is required to keep current claims active (no +1)
        final int claimWeight   = FactionsMod.CONFIG.POWER.CLAIM_WEIGHT;
        final int requiredPower = io.icker.factions.util.FactionsSafe.claimCount(faction) * claimWeight;
        final int maxPower      = users.size() * FactionsMod.CONFIG.POWER.MEMBER + FactionsMod.CONFIG.POWER.BASE;
        final boolean ok        = faction.getPower() >= requiredPower;
        final String color      = ok ? Formatting.GREEN.toString() : Formatting.RED.toString();

// Resolve names from online players; fallback to a placeholder if offline/uncached
        final var pm = player.getEntityWorld().getServer().getPlayerManager();
        ...
        String owner =
                Formatting.WHITE
                        ...
                        .map(u -> {
                            ServerPlayerEntity online = pm.getPlayer(u.getID());
                            return online != null
