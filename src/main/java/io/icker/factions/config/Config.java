...
public class Config {
    ...
    public static Config load() {
        Gson gson =
                new GsonBuilder()
                        ...
                        .registerTypeAdapter(DecayConfig.class, new Deserializer<>(DecayConfig.class))  // added
                        .create();
    ...
    /** Configuration for faction decay based on inactivity */
    public static class DecayConfig {
        @SerializedName("enabled")
        public boolean enabled = true;

        /** Inactivity threshold in days before decay begins */
        @SerializedName("inactiveThresholdDays")
        public int inactiveThresholdDays = 3;

        /** Amount of power drained per interval (see drainIntervalHours) */
        @SerializedName("drainAmount")
        public int drainAmount = 1;

        /** Interval in hours between each drain tick */
        @SerializedName("drainIntervalHours")
        public int drainIntervalHours = 1;

        /** Amount of power each new member contributes (for tracking maximum drain) */
        @SerializedName("powerPerMember")
        public int powerPerMember = 20;
    }
