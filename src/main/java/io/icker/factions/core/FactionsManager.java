...
public class FactionsManager {
    ...
    public static void register() {
        ...
        if (FactionsMod.CONFIG.HOME != null && FactionsMod.CONFIG.HOME.CLAIM_ONLY) {
            ClaimEvents.REMOVE.register((x, z, level, faction) -> {
                ...
                if (home == null || !Objects.equals(home.level, level)) return;
    ...
    private static void factionModified(Faction faction) {
        ServerPlayerEntity[] players = faction.getUsers().stream()
                .map(u -> playerManager.getPlayer(u.getID()))
                .filter(p -> p != null)
    ...
    private static void memberChange(Faction faction, User user) {
        ...
        if (player != null) updatePlayerList(player);
    }
    ...
    private static void playerDeath(ServerPlayerEntity player, DamageSource source) {
        ...
        // cap death penalty by user's remaining contribution and faction power
        int drainedSoFar    = user.getPowerDrained();
        int maxDrain        = Config.load().DECAY.powerPerMember;
        int penaltyPerDeath = FactionsMod.CONFIG.POWER.DEATH_PENALTY;
        int currentPower    = faction.getPower();

        int remainingForUser = maxDrain - drainedSoFar;
        int actualPenalty    = Math.min(penaltyPerDeath,
                Math.min(remainingForUser, currentPower));

        if (actualPenalty > 0) {
            int adjusted = faction.adjustPower(-actualPenalty);
            user.setPowerDrained(drainedSoFar + actualPenalty);
    ...
    private static ActionResult openSafe(PlayerEntity player, Faction faction) {
        ...
        player.openHandledScreen(new SimpleNamedScreenHandlerFactory(
                (syncId, inventory, p) -> FactionsMod.CONFIG.SAFE.DOUBLE
                        ? GenericContainerScreenHandler.createGeneric9x6(syncId, inventory, faction.getSafe())
